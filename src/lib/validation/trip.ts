...
export const tripRequestSchema = z
  .object({
    ...
    start_date: z
      ...
      .refine((value) => value >= todayInVietnam(), "Ngày khởi hành không được ở quá khứ"),
  ...
  .superRefine((value, ctx) => {
    if (value.adults + value.children + value.elderly > 30) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["adults"],
        message: "Tổng số người không được vượt quá 30",
      });
    }
  });

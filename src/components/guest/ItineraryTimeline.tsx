...
export function ItineraryTimeline({
  ...
  function requestReplacement(product: TravelProduct) {
    const text = `Thay dịch vụ "${product.name}" bằng một lựa chọn tương tự, phù hợp hơn với nhu cầu của tôi.`;
    window.dispatchEvent(
      new CustomEvent("nibigo:prefill-revision", { detail: { text } }),
    );
    document.getElementById("refine")?.scrollIntoView({
      behavior: "smooth",
      block: "start",
    });
  }
  ...
  if (itinerary.length === 0) {
    return (
      <div className="rounded-[24px] border border-dashed border-black/10 bg-white p-6 text-sm leading-7 text-brand-muted">
        Chưa có lịch trình chi tiết. Bạn có thể quay lại 3 phương án hoặc gửi yêu cầu để được tư vấn viên hỗ trợ rõ hơn.
      </div>
    );
  }
          ...
          <header className="flex flex-col gap-3 border-b border-black/5 bg-brand-green-soft/[0.55] px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <h3 className="mt-0.5 text-lg font-bold text-brand-ink">
                  {day.title}
                </h3>
              </div>
            </div>
            ...
            {day.slots.map((slot, index) => {
              ...
              return (
                <li
                  ...
                  className="grid gap-3 py-5 sm:grid-cols-[92px_1fr]"
                >
                  ...
                  <div className="rounded-[20px] border border-black/5 bg-[#fffdf9] p-4">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        {product && (
                          <div className="mt-4 flex flex-wrap items-center gap-3 border-t border-black/5 pt-3">
                            ...
                            <button
                              type="button"
                              onClick={() => requestReplacement(product)}
                              className="text-xs font-bold text-brand-muted hover:text-brand-green"
                            >
                              Thay dịch vụ này
                            </button>
                          </div>
